...
//save unordered mesh # only of the surface
event vtu(t+=0.2;t<Tmax){
  char filename[40];
  sprintf(filename, "vtu/TIME-%06g", (t*100));
  output_vtu((scalar *) {eta}, (vector *) {u}, filename);
}

event vtu_3d(t=T0*0.5, last){
  char filename[40];
  sprintf(filename, "vtu3d/TIME-%06g", (t));
  //vertical_remapping(beta, u.x);
  vertical_remapping(h, (scalar *) {eta, u});
  output_vtu((scalar *) {eta}, (vector *) {u}, filename);
}
