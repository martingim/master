...
def StokesDispSolver(**kwargs):
    ...
    if 'a' in kwargs and 'T' in kwargs:
        ...
        if modeNo == 1:
            ...
            kResults = [k.evalf() for k in kResults]
            Results['a'] = a
            ...
            omega4 = (a0**4 * k**4 * (20 * alpha1**5 + 112 * alpha1**4 - 100 * alpha1**3 - 68 * alpha1**2 - 211 * alpha1 + 328)) / (32 * (alpha1 - 1)**5)
            omegaFun = omega0 * (1 + omega2 + omega4) - omega
            S = sp.solve([a0Fun, omegaFun], [k, a0], [2 * sp.pi / L0, a])
            kResults = abs(S[0][0].evalf())
            aResults = abs(S[0][1].evalf())
            Results['a'] = aResults
            Results['aw'] = a
            a = Results['a']
        Results['k'] = kResults
        Results['omega0'] = sp.sqrt(g * kResults * sp.tanh(kResults * h))
    elif 'H' in kwargs and 'T' in kwargs:
        omega0 = sp.sqrt(g * k * sp.tanh(k * h))
        Results['T'] = T
        omega = 2 * sp.pi / T
        Results['omega'] = omega
        ...
        Results['omega0'] = sp.sqrt(g * kResults * sp.tanh(kResults * h))
    elif 'a' in kwargs and 'omega0' in kwargs:
        ...
        B53 = (57 * sp.cosh(2 * h * kResults[0])**7 + 204 * sp.cosh(2 * h * kResults[0])**6 - 53 * sp.cosh(2 * h * kResults[0])**5
               ...
               + 371 * sp.cosh(2 * h * kResults[0]) + 186) * 9 / ((3 * sp.cosh(2 * h * kResults[0]) + 2) * (sp.cosh(2 * h * kResults[0]) - 1)**6 * 128)
